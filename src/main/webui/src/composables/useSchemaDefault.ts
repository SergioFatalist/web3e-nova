import { z, ZodDefault, ZodEffects, ZodOptional, ZodType } from "zod";

export type ZodSchemaKeysFrom<E> = {
  [k in keyof E]:
    | ZodType<E[k]>
    | ZodDefault<ZodType<E[k]>>
    | ZodEffects<ZodDefault<ZodType<E[k]>>>
    | ZodOptional<ZodEffects<ZodDefault<ZodType<E[k]>>>>
    | ZodOptional<ZodType<E[k]>>
    | ZodDefault<ZodOptional<ZodType<E[k]>>>
    | ZodEffects<ZodOptional<ZodType<E[k]>>>;
};
...
type DefaultGetter<T extends SupportedZodTypes> = T extends SimpleSupportedZodTypes
  ? BaseDefaultGetter
...
const defaultZodValueGetterMap = new Map<string, DefaultGetter<SupportedZodTypes>>();
...
defaultZodValueGetterMap.set(z.ZodDefault.name, (f: z.ZodDefault<any>) => f._def.defaultValue());
...
defaultZodValueGetterMap.set(z.ZodUnion.name, (f: z.ZodUnion<any>) =>
  getSchemaDefaultForField(f._def.options as SupportedZodTypes)
);
defaultZodValueGetterMap.set(z.ZodObject.name, (f: z.ZodObject<any>) => getSchemaDefault(f));
defaultZodValueGetterMap.set(z.ZodRecord.name, (f: z.ZodRecord<any>) => getSchemaDefault(f));
defaultZodValueGetterMap.set(z.ZodIntersection.name, (f: z.ZodIntersection<any, any>) =>
  getSchemaDefault(f)
);
 ...
 */
function getSchemaDefaultForField<T extends SupportedZodTypes>(field: T): z.infer<T> | undefined {
  const typeKey = field.constructor.name;
  if (!defaultZodValueGetterMap.has(typeKey)) {
    console.warn("getSchemaDefaultForField: Unhandled type", field.constructor.name);
 ...
 */
export function getSchemaDefault<T extends SupportedZodTypes>(schema: T): z.infer<T> {
  ...
  if (isOfType<z.ZodIntersection<z.ZodTypeAny, z.ZodTypeAny>>(schema, z.ZodIntersection)) {
    return {
      ...getSchemaDefault(schema._def.left as SupportedZodTypes),
      ...getSchemaDefault(schema._def.right as SupportedZodTypes),
    } as z.infer<T>;
  }
  ...
  if (isOfType<z.ZodUnion<[z.ZodTypeAny, ...z.ZodTypeAny[]]>>(schema, z.ZodUnion)) {
    for (const option of schema._def.options) {
      if (isOfType<z.ZodObject<z.ZodRawShape>>(option, z.ZodObject)) {
        return getSchemaDefault(option) as z.infer<T>;
      }
    }

    console.warn("getSchemaDefaultObject: No object found in union, returning empty object");
    return {};
  }

  if (!isOfType<z.ZodObject<z.ZodRawShape>>(schema, z.ZodObject)) {
    console.warn(`getSchemaDefaultObject: Expected object schema, got ${schema.constructor.name}`);
    return {};
  }

  return Object.fromEntries(
    Object.entries(schema.shape as Record<string, SupportedZodTypes>).map(([key, field]) => [
      key,
...
export default function useSchemaDefault<
