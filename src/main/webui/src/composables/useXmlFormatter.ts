...
function processContent(content: string, state: XMLFormatterState, preserveSpace: boolean): void {
  if (!preserveSpace) {
    ...
    if (state.options.lineSeparator) {
      ...
      content = decodeURIComponent(trimmedContent);
    }
  }
  if (content.length > 0) {
...
function processElementNode(
  node: XmlParserElementNode,
  state: XMLFormatterState,
  ...
  if (
    node.children === null ||
    (state.options.forceSelfClosingEmptyTag && node.children.length === 0)
  ) {
...
function processProcessingIntruction(
  node: XmlParserProcessingInstructionNode,
  state: XMLFormatterState
): void {
