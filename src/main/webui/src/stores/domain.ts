...
import { z } from "zod";
import { domainMessageSchema } from "@/generated/schema/callmanager/domain";
import type { DomainMessage } from "@/generated/protobuf/callmanager/domain";

export type DomainsViewState = ViewState<
  DomainMessage,
  BaseItemMessage,
  typeof domainMessageSchema
> & {
  dictionary: DictionaryItem[];
};

export const useDomainStore = defineStore("domain", {
  state: (): DomainsViewState => ({
    viewUrl: "/pbx-admin/directory/domain",
    apiUrl: "/api/callmanager/domain",
    loading: false,
    pagination: {
      page: 1,
      rowsPerPage: 10,
    ...
    item: clone(useSchemaDefault(domainMessageSchema)),
    validationSchema: domainMessageSchema,
    validationErrors: {} as Partial<Record<keyof typeof domainMessageSchema, string>>,
    dictionary: [],
  }),
  persist: {
  ...
  actions: {
    ...
    async validateForm() {
      try {
        ...
        this.validationErrors = {} as UnwrapRef<
          Partial<Record<keyof typeof domainMessageSchema, string>>
        >;
        return true;
      } catch (error) {
        if (error instanceof z.ZodError) {
          this.validationErrors = error.errors.reduce(
            (acc, curr) => {
              const path = curr.path[0] as keyof z.infer<typeof domainMessageSchema>;
    ...
    async create() {
      this.item = clone(useSchemaDefault(domainMessageSchema));
    ...
    async get(id: string) {
      try {
        if (id == NIL) return;
        const response = await axiosInstance.post<DomainMessage>(`${this.apiUrl}/get`, { id });
        this.$patch({
          item: response.data,
        });
      } catch (e) {
