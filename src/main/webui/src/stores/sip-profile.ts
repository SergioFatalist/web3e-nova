...
import { z } from "zod";
import { sipProfileMessageSchema } from "@/generated/schema/callmanager/sip-profile";
import type { SipProfileMessage } from "@/generated/protobuf/callmanager/sip-profile";

export type SipProfilesViewState = ViewState<
  SipProfileMessage,
  BaseItemMessage,
  typeof sipProfileMessageSchema
> & {
  dictionary: DictionaryItem[];
};

export const useSipProfileStore = defineStore("sip-profile", {
  state: (): SipProfilesViewState => ({
    viewUrl: "/pbx-admin/configuration/sip-profile",
    apiUrl: "/api/callmanager/sip-profile",
    loading: false,
    pagination: {
      page: 1,
      rowsPerPage: 10,
    ...
    item: clone(useSchemaDefault(sipProfileMessageSchema)),
    validationSchema: sipProfileMessageSchema,
    validationErrors: {} as Partial<Record<keyof typeof sipProfileMessageSchema, string>>,
    dictionary: [],
  }),
  persist: {
  ...
  actions: {
    ...
    async validateForm() {
      try {
        ...
        this.validationErrors = {} as UnwrapRef<
          Partial<Record<keyof typeof sipProfileMessageSchema, string>>
        >;
        return true;
      } catch (error) {
        if (error instanceof z.ZodError) {
          this.validationErrors = error.errors.reduce(
            (acc, curr) => {
              const path = curr.path[0] as keyof z.infer<typeof sipProfileMessageSchema>;
    ...
    async create() {
      this.item = clone(useSchemaDefault(sipProfileMessageSchema));
      await this.$router.push(`${this.viewUrl}/create/${NIL}`);
    },

    async prepare() {
      const response = await axiosInstance.get<SipProfileMessage>(`${this.apiUrl}/prepare`);
    ...
    async get(id: string) {
      try {
        if (id == NIL) return;
        const response = await axiosInstance.post<SipProfileMessage>(`${this.apiUrl}/get`, { id });
        this.$patch({
          item: response.data,
        });
      } catch (e) {
