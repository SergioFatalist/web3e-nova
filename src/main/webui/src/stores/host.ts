...
import { z } from "zod";
import type { HostMessage } from "@/generated/protobuf/callmanager/host";
import { hostMessageSchema } from "@/generated/schema/callmanager/host";

export type HostsViewState = ViewState<HostMessage, BaseItemMessage, typeof hostMessageSchema> & {
  freeswitchXml?: string;
  overlay: boolean;
  dictionary: DictionaryItem[];
};

export const useHostStore = defineStore("host", {
  state: (): HostsViewState => ({
    viewUrl: "/pbx-admin/configuration/host",
    apiUrl: "/api/callmanager/host",
    loading: false,
    overlay: false,
    pagination: {
      page: 1,
    ...
    item: clone(useSchemaDefault(hostMessageSchema)),
    validationSchema: hostMessageSchema,
    validationErrors: {} as Partial<Record<keyof typeof hostMessageSchema, string>>,
    dictionary: [],
  }),
  persist: {
  ...
  actions: {
    ...
    async validateForm() {
      try {
        ...
        this.validationErrors = {} as UnwrapRef<
          Partial<Record<keyof typeof hostMessageSchema, string>>
        >;
        return true;
      } catch (error) {
        if (error instanceof z.ZodError) {
          this.validationErrors = error.errors.reduce(
            (acc, curr) => {
              const path = curr.path[0] as keyof z.infer<typeof hostMessageSchema>;
    ...
    async prepare() {
      const response = await axiosInstance.get<HostMessage>(`${this.apiUrl}/get`);
    ...
    async get(id: string) {
      try {
        if (id == NIL) return;
        const response = await axiosInstance.post<HostMessage>(`${this.apiUrl}/get`, { id });
        this.$patch({
          item: response.data,
        });
      } catch (e) {
    ...
    async getFreeswitchXml(id: string) {
      try {
        const response = await axiosInstance.get(`${this.apiUrl}/get-freeswitch-xml`, {
          params: { id },
        });
        this.freeswitchXml = response.data;
      } catch (e) {
        useAxiosError(<AxiosError>e);
      }
    },
